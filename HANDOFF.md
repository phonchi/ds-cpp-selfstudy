...
`b97fe81` 讀回九頁 HTML、九份題庫、九份字卡，接著做局部修正。保留重要 C++ 主題、
原表格、每頁背景 SVG 與原版播放器；頁內與末尾習題一律四選一，字卡保留英文。

以 [prereq-original-restoration.md](docs/prereq-original-restoration.md) 為本版的範圍與驗收紀錄。
`lesson-trace`、舊的 109 題／238 卡等數字與「恢復了原稿」的先前宣稱，均不再描述目前版本。
正課九章以本輪開始的 `a9425db` 保留，沒有回退正課改動；使用者已授權修完直接推送發布。

維護時直接編輯原版 HTML 區塊；不要使用整頁 authoring helper 取代播放器或圖表。
JSON 同步需指定 `--pages`，避免無意重生正課。fidelity checker 的基準是原 commit，
完整程式也要檢查原 `.pseudo-code` DIV，不能只測少數 `pre[data-cpp]`。
...
C++ 語義與測驗正確性，只換課本耦合、課程統計並依先備重排。完整範圍與跨組閱讀記錄見
[prereq-coverage-20260906.md](docs/prereq-coverage-20260906.md)。

P1–P9 均改回主文設計：沒有第二套 root/hero/container 全域覆寫。每頁至少兩個
`.lesson-trace`，程式行、狀態與累積輸出一起追蹤；主文元件、詞卡鍵盤操作與選項洗牌保留。
`check_prereq_browser.py` 現檢查所有 trace，不再使用每頁單一固定播放器 ID；並與主文
computed style 比較。`check_prereq.py` 新增真正多檔分組編譯。HTML 仍是正文唯一來源，
新增的 authoring helper 只是可選寫作輔助，不是線上依賴。

本版驗收：85 個完整程式、1 組多檔、21 個互動、99 題全部選項、149 張詞卡通過；
九頁桌面／手機 computed style 與主文一致，連結、對比與 diff 檢查均無錯。


### 前次精簡版（05f490e，已由完整教材取代）
...
7. **Windows 自帶工具鏈**（`jcppkernel/toolchain.py`、`setup_cli.py`，commit `0ce2ce4` + 檔案鎖 `94feb5e`（兩個 kernel 同時首次啟動只下載一次；stale lock 30 分鐘自動清），zip sha256 `6b957b84…3aaa`）：找 g++ 的順序 `JCPP_GXX` → PATH 上現成的 → 受管
   ...
   sha256 `aeec29c2…3d7a6`）；② winlibs 官方 198 MB 原版。8 條連線的 4 MB 小塊工作佇列下載（停滯 10 s 重抓、尾端 2 s 後重複請求，commit `a766542`），終端機與 notebook 都有進度條。
...
| `tools/fix_bare_include.py` | 講義範例卡裸檔名 `#include "X.hpp"` → `"pythonds3/cppds/X.hpp"`（只改課程標頭目錄裡存在的檔名）；標頭目錄可用 `DSCPP_HEADERS` 覆寫，預設目錄不存在時警告略過（exit 0） | 改過就不再命中 |
